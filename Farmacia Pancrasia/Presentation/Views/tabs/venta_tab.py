    ...
    def decrementar_cantidad(self):
        ...
        if selected:
            if int(self.tree_productos.item(selected)["values"][3]) > 1:
                self.tree_productos.item(selected, values=(self.tree_productos.item(selected)["values"][0], self.tree_productos.item(selected)["values"][1], self.tree_productos.item(selected)["values"][2], Decimal(self.tree_productos.item(selected)["values"][3])+1))
                self.label_total.config(text="Total: $"+str(self.get_total()))
            else:
            ...
            if producto:
                ...
                for child in self.tree_productos.get_children():
                    if self.tree_productos.item(child)["values"][1] == producto.descripcion:
                        ...
                        self.tree_productos.item(child, values=(producto.codigo, producto.descripcion, producto.precio, cantidad))
